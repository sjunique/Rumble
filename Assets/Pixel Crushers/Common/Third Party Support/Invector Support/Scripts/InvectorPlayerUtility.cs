...
namespace PixelCrushers.InvectorSupport
{
    ...
    public static class InvectorPlayerUtility
    {
        ...
        public static void PausePlayer()
        {
            if (input != null)
            {
                var controller = input.GetComponent<vThirdPersonController>();
                controller.StopCharacter();
#if USE_INVECTOR_FREE
                input.cc.animator.SetFloat(vAnimatorParameters.InputHorizontal, 0);
                input.cc.animator.SetFloat(vAnimatorParameters.InputVertical, 0);
                input.cc.animator.SetFloat(vAnimatorParameters.InputMagnitude, 0);
                ...
                //--- Disabling in 2.6.4 disables collider, which makes player sink into floor.
                //--- input.enabled = false;
#endif
